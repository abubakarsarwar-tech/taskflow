...
import { Task, Notification, sequelize } from './models/index.js';
import {
  SERVER_CONFIG,
  CORS_CONFIG,
  validateEnv
...
const checkDeadlines = async (io) => {
  try {
    ...
    try {
      ...
      console.log('⏰ Skipping deadline check - database not connected');
      return;
    }
    ...
    const { Board } = await import('./models/index.js');
    const tasks = await Task.findAll({
      where: {
        dueDate: {
          [sequelize.Sequelize.Op.gte]: now,
          [sequelize.Sequelize.Op.lte]: tomorrow
        },
        status: {
          [sequelize.Sequelize.Op.ne]: 'done'
        }
      },
    ...
    for (const task of tasks) {
      ...
      if (!existing && task.board) {
        const notification = await Notification.create({
          userId: task.userId,
          message: `Deadline approaching: "${task.title}" in board "${task.board.name}" is due within 24 hours!`,
          type: 'task_deadline',
        ...
        if (io) {
          io.to(`user:${task.userId}`).emit('new_notification', notification.toJSON());
        }
      }
    }
  } catch (error) {
