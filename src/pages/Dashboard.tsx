...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (currentBoardId) {
      console.log('🎯 Dashboard: currentBoardId changed to:', currentBoardId);
  ...
  // Check for boardId, invite, or taskId in URL
  useEffect(() => {
    ...
    if (taskId && safeTasks[taskId]) {
      const task = safeTasks[taskId];
      setViewingTask(task);
      setIsDetailModalOpen(true);

      // Clear taskId after opening using a functional update to avoid dependency on searchParams
  ...
  }, [currentBoardId, setCurrentBoard, safeTasks, setSearchParams]); // Removed searchParams to avoid possible circular notification
