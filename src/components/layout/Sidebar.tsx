...
export function Sidebar({ onOpenSettings }: SidebarProps) {
  ...
  const accessibleBoards = boards.filter((board) => {
    ...
    // User is in the members array (with null check)
  ...
  const getDefensiveBoardColor = (boardId: any) => {
    const colors = [
    ...
    const index = idStr.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % colors.length;
    return colors[index];
  };
                  ...
                  >
                    <button
                      ...
                      className={cn(
                        ...
                        getDefensiveBoardColor(board.id)
                      )}
                    >
                      <div className={cn(
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                        {(isAdmin || board.ownerId === user?.id || (board.ownerId as any)?._id === user?.id || (board.members && Array.isArray(board.members) && board.members.find(m => m.id === user?.id)?.role === 'admin')) && (
                          <DropdownMenuItem
                            onClick={(e) => {
                              e.stopPropagation();
