            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  <Button
                    ...
                    size="sm"
                    className={cn(
                      "h-7 px-2 flex items-center gap-1 text-muted-foreground rounded-full transition-colors",
                  ...
                  >
                    ...
                    {(task.commentsCount || 0) > 0 && (
                      <span className="text-xs font-bold pointer-events-none">{task.commentsCount}</span>
                    )}
                  </Button>
              ...
              <div className="flex items-center justify-between mt-4 pt-3 border-t border-border/50">
                <div className="flex items-center gap-2">
                  <Badge variant="outline" className={cn('text-xs border rounded-full', priorityConfig.className)}>
                    <Flag className="h-3 w-3 mr-1 pointer-events-none" />
                    {priorityConfig.label}
                  </Badge>

                  {dueDateStatus && (
                    <Badge variant="outline" className={cn('text-xs rounded-full', dueDateStatus.className)}>
                      <Calendar className="h-3 w-3 mr-1 pointer-events-none" />
                      {dueDateStatus.label}
                    </Badge>
                  )}
                </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    {task.assignee.avatar ? (
                      <img
                        ...
                        className="h-6 w-6 rounded-full ring-2 ring-border pointer-events-none"
                      />
                    ) : (
                      <div className="h-6 w-6 rounded-full bg-primary/20 flex items-center justify-center ring-2 ring-border">
                        <User className="h-3 w-3 text-primary pointer-events-none" />
                      </div>
                    )}
                  </div>
                )}
