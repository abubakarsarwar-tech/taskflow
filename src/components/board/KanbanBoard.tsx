...
export function KanbanBoard({ board, filteredTaskIds, onEditTask, onViewTask }: KanbanBoardProps) {
  const { user } = useAuth();
  ...
  const [isInternalTaskModalOpen, setIsInternalTaskModalOpen] = useState(false);
  const [internalEditingTask, setInternalEditingTask] = useState<Task | null>(null);
  ...
  const isOwner = String(boardOwnerId) === String(user?.id);
  const userMember = board.members.find((m) => String(m.id) === String(user?.id));
  ...
  return (
    <>
      ...
      {/* Internal Task Creation Modal */}
      <TaskModal
        open={isInternalTaskModalOpen}
        onOpenChange={setIsInternalTaskModalOpen}
