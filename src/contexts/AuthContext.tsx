...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (user && token) {
      ...
      // This prevents connection errors during OAuth redirects
      const initTimeout = setTimeout(() => {
        try {
          useTaskStore.getState().initializeSocket(token, user.id);
        } catch (error) {
          console.warn('🔌 Socket initialization failed, but app will continue:', error);
          // Don't throw - allow app to work without real-time features
        }
      }, 500);
...
4. Verify CORS is configured correctly`;
  ...
  const completeOAuthLogin = (token: string, userData: { id: any, email: string, name: string, role?: 'admin' | 'user' }) => {
